...
class Round:
    ...
    def handle_round_recess(self):
        ...
        try:
            server_api.post_round_data(
                self.gsr.id if not self.gsr.is_guest else 0,
                self.ex.id if not self.ex.is_guest else 0,
                self.__chosen_word,
                self.p1scoreboard["ROUND_TOTAL"] if self.p1 == self.gsr else self.p2scoreboard["ROUND_TOTAL"]
            )
    ...
    def player_choose_word(self, content : str) -> tuple:
        ...
        if content == "__RANDOM__":
            ...
            if len(content) <= 2:
                ...
                return (True, "warning", "Word contains invalid character(s) or is inappropriate. Please choose again.")
    ...
    def player_choose_hint(self, content):
        ...
        if content not in self.__hints and content != "__RANDOM__":
            ...
            notif = {
                ...
                "content": "New hint has been added!",
                "tag": "HINT_UPDATED"
            }
    ...
    def start_round(self, p1gt=0, p2gt=0):
        ...
        self.update_public_data("hints", [])
        self.update_public_data("hint_requested", False)
        self.update_public_data("p1scoreboard", {"ID": self.p1.id, "NAME": self.p1.name, "FINISHED_GUESSING": False, "GAME_TOTAL" : p1gt, "DISCONNECTED" : False})
        self.update_public_data("p2scoreboard", {"ID": self.p2.id, "NAME": self.p2.name, "FINISHED_GUESSING": False, "GAME_TOTAL" : p2gt, "DISCONNECTED" : False})
