...
from server_api import post_game_data, update_player_data
...
class Game:
    ...
    def start_game(self):
        self.handle_chat("SERVER", {"content": "WARNING: This game uses unmonitored outside resources that might return inappropriate content. Player discretion is advised."})
    ...
    def handle_game_end(self):
        ...
        p1score = self.current_round.p1scoreboard["GAME_TOTAL"]
        p2score = self.current_round.p2scoreboard["GAME_TOTAL"]
        if p1score == p2score and p1score == 0:
            return

        if not self.p1.is_guest:
            won = True if p1score >= p2score else False
            won = False if self.p1.invalid_authorization else won
            post_game_data(
                self.p1.id,
                self.p2.id if not self.p2.is_guest else 0,
                won,
                p1score,
                p2score,
            )
        ...
        if not self.p2.is_guest and not self.p2.invalid_authorization:
            won = True if p2score >= p1score else False
            won = False if self.p2.invalid_authorization else won
            post_game_data(
                self.p2.id,
                self.p1.id if not self.p1.is_guest else 0,
                won,
                p2score,
                p1score,
            )
